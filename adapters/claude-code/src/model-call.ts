/**
 * Model-call adapter for the Claude Code hook. Talks to Gemini or Anthropic
 * over plain fetch — no SDK dependency. Provider is encoded in the model
 * string as `<provider>/<model-id>`, matching the OpenClaw adapter's pattern.
 *
 * Keys are read from process.env. Typical layout (set in ~/io-data/.env or in
 * hooks.json):
 *   GEMINI_API_KEY=...
 *   ANTHROPIC_API_KEY=...
 */
