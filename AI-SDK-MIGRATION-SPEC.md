...
| Axis | Anthropic auto mode | io-auto-mode |
|---|---|---|
...
| **Prompt-injection probe** | server-side input probe | N/A (classifies command pre-exec; different philosophy) |
| **Setup** | flip a flag | build + install + config |

### What Anthropic's release arguably obsoletes

1. **Bash classification on vanilla Claude Code for Max users who don't need local models.** Their release covers the 80% case natively.
2. **The "we're the only auto mode" framing.** Positioning shifts from *auto mode* to *self-hosted / multi-platform / local-first auto mode*.
...
Once ai-sdk migration lands, consider:
...
No changes required. Phases 0-6 stand as written. Only phase 6 ("Update INSTALL.md + related specs") gains: also update `SPEC.md` and `BACKLOG.md` to reflect the positioning shift above.
